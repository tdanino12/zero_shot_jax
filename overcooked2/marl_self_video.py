...
from jaxmarl.viz.overcooked_v2_visualizer import OvercookedV2Visualizer  # ADDED
...
def get_rollout(train_state, config):
    ...
    # FIX: build init_x from a real reset observation instead of the
    # declared observation_space().shape, so the network is always
    # initialized with the actual obs dimension the env produces
    # (this avoids the ScopeParamShapeError when a layout's declared
    # obs shape doesn't match what get_obs() actually returns).
    init_obs, _ = env.reset(key_r)
    init_x = init_obs[env.agents[0]].flatten()
    ...
    analsis_info_agent_0 = {
                        "plate pickup": 0,
                        "ingredient_0 picked": 0,
                        "ingredient_1 picked": 0,
                        "ingredient_2 picked": 0,
                        "dish picked": 0,
                        "delivery": 0,
                        "plate placed on counter": 0,
                        "ingredient_0 placed on counter": 0,
                        "ingredient_1 placed on counter": 0,
                        "ingredient_2 placed on counter": 0,
                        "ingredient_0 placed in pot": 0,
                        "ingredient_1 placed in pot": 0,
                        "ingredient_2 placed in pot": 0,
                        "dish placed on counter": 0,
                        "total_reward": 0,
                        "reward": 0
    }
    analsis_info_agent_1 = {
                        "plate pickup": 0,
                        "ingredient_0 picked": 0,
                        "ingredient_1 picked": 0,
                        "ingredient_2 picked": 0,
    ...
    while not done:
        ...
        # Added analsys
        a0 = info["agent_0_info"]
        a1 = info["agent_1_info"]

        # Scalar events
        analsis_info_agent_0["plate pickup"]           += int(a0["plate pickup"])
        analsis_info_agent_1["plate pickup"]           += int(a1["plate pickup"])
        analsis_info_agent_0["dish picked"]            += int(a0["dish picked"])
        analsis_info_agent_1["dish picked"]            += int(a1["dish picked"])
        analsis_info_agent_0["delivery"]               += int(a0["delivery"])
        analsis_info_agent_1["delivery"]               += int(a1["delivery"])

        # plate placed on counter: only count when no delivery happened that step
        if a0["delivery"] == 0:
            analsis_info_agent_0["plate placed on counter"] += int(a0["plate placed on counter"])
        if a1["delivery"] == 0:
            analsis_info_agent_1["plate placed on counter"] += int(a1["plate placed on counter"])

        # dish placed on counter: only count when no delivery happened that step
        if a0["delivery"] == 0:
            analsis_info_agent_0["dish placed on counter"] += int(a0["dish placed on counter"])
        if a1["delivery"] == 0:
            analsis_info_agent_1["dish placed on counter"] += int(a1["dish placed on counter"])

        # Per-ingredient events (arrays of shape MAX_INGREDIENTS)
        for i in range(len(a0["ingredient picked"])):
            # ingredient picked
            analsis_info_agent_0[f"ingredient_{i} picked"] += int(a0["ingredient picked"][i])
            analsis_info_agent_1[f"ingredient_{i} picked"] += int(a1["ingredient picked"][i])

            # ingredient placed in pot
            analsis_info_agent_0[f"ingredient_{i} placed in pot"] += int(a0["ingredient placed in pot"][i])
            analsis_info_agent_1[f"ingredient_{i} placed in pot"] += int(a1["ingredient placed in pot"][i])

            # ingredient placed on counter: only count when it wasn't also put in pot that step
            if a0["ingredient placed in pot"][i] == 0:
                analsis_info_agent_0[f"ingredient_{i} placed on counter"] += int(a0["ingredient placed on counter"][i])
            if a1["ingredient placed in pot"][i] == 0:
                analsis_info_agent_1[f"ingredient_{i} placed on counter"] += int(a1["ingredient placed on counter"][i])

        analsis_info_agent_0["reward"] += info["individual_rewards"]["agent_0"]
        analsis_info_agent_1["reward"] += info["individual_rewards"]["agent_1"]

        total_reward = analsis_info_agent_0["reward"] + analsis_info_agent_1["reward"]
        analsis_info_agent_0["total_reward"] = total_reward
        analsis_info_agent_1["total_reward"] = total_reward


        state_seq.append(state)

    return state_seq, analsis_info_agent_0, analsis_info_agent_1


# ADDED
def render_state_seq(state_seq, env):
    # v2 states (grid/agents/recipe pytree) aren't compatible with the v1
    # OvercookedVisualizer (which expects maze_map/agent_dir_idx/agent_inv).
    # OvercookedV2Visualizer.render_sequence handles v2 states directly, but
    # it's vmapped internally, so state_seq (currently a plain Python list of
    # per-step State pytrees) needs to be stacked into one batched pytree first.
    stacked_states = jax.tree_util.tree_map(lambda *xs: jnp.stack(xs), *state_seq)
    viz = OvercookedV2Visualizer()
    frames = viz.render_sequence(stacked_states, agent_view_size=None)  # (T, H, W, C) uint8
...
def make_train(config):
    ...
    print("LINE:", inspect.getsourcelines(fn)[1])
    print("##################################################################")
    env = LogWrapper(env)
    ...
    def train(rng):
        ...
        # FIX: build init_x from a real reset observation instead of the
        # declared observation_space().shape. This guarantees the network's
        # first-layer kernel is sized to match exactly what env.reset()/env.step()
        # actually return, which is what caused the ScopeParamShapeError
        # (680 vs 700) when the declared obs shape didn't match the real one
        # for this layout.
        _rng, _rng_reset = jax.random.split(_rng)
        init_obs, _ = env.reset(_rng_reset)
        init_x = init_obs[env.agents[0]].flatten()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total_timesteps", type=int, default=35e6)#35e6) # 55e6
    parser.add_argument("--update_epochs", type=int, default=20)
    parser.add_argument("--num_minibatches", type=int, default=4)
    parser.add_argument("--gamma", type=float, default= 0.99)
    parser.add_argument("--gae_lambda", type=float, default=0.95)
    parser.add_argument("--clip_eps", type=float, default=0.2)
    parser.add_argument("--ent_coef", type=float, default=0.01)#0.01)
    parser.add_argument("--vf_coef", type=float, default=0.5)
    parser.add_argument("--max_grad_norm", type=float, default=0.5)
    parser.add_argument("--activation", type=str, default="tanh")
    parser.add_argument("--env_name", type=str, default="overcooked_v2")
    parser.add_argument("--anneal_lr", type=bool, default=True)
    parser.add_argument("--output", type=str, default="self_play")
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--layout", type=str, default="asymm_advantages_recipes_center")
    ...
    config = {
        "env_kwargs": {
            "layout": overcooked_v2_layouts[args.layout],
        },
        "num_seeds": 1,
    }
    ...
    state_seq,info_0,info_1 = get_rollout(train_state, config)

    print("#########################################################")
    print("agent 0 (main agent) info:")
    print(info_0)
    print("#########################################################")
    print("agent 1 info:")
    print(info_1)
    ...
    path = f"csv_files/{args.layout}/phase1_lr/{args.seed}"
    os.makedirs(path, exist_ok=True)
    with open(f"csv_files/{args.layout}/phase1_lr/{args.seed}/output.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=info_0.keys())
        writer.writeheader()
        writer.writerow(info_0)
