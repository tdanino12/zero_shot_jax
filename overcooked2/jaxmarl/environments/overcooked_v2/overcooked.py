...
class OvercookedV2(MultiAgentEnv):
    ...
    def step_env(
        ...
        state, reward, shaped_rewards, agent_infos = self.step_agents(key, state, acts)
        ...
        # Individual reward: in v2, delivery reward is shared (single scalar).
        # We mirror the v1 convention by attributing it per-agent from the scan.
        individual_rewards = {f"agent_{i}": reward for i in range(self.num_agents)}

        # Per-agent analysis info: agent_infos is a dict of stacked arrays,
        # one value per agent (index 0 = agent_0, index 1 = agent_1).
        agent_0_info = {k: v[0] for k, v in agent_infos.items()}
        agent_1_info = {k: v[1] for k, v in agent_infos.items()}
        ...
        return (
            ...
            dones,
            {
                ...
                "agent_0_info": agent_0_info,
                "agent_1_info": agent_1_info,
            },
        )
    ...
    def step_agents(
        ...
        def _interact_wrapper(carry, x):
            ...
            def _interact(carry, agent):
                ...
                return carry, (new_agent, shaped_reward, analsis_info)

            # When the agent does NOT interact, return zero-valued analsis_info
            # with the same structure so jax.lax.scan sees a consistent pytree.
            zero_info = {
                "plate pickup":             jnp.array(0, dtype=jnp.bool_),
                "dish picked":              jnp.array(0, dtype=jnp.bool_),
                "delivery":                 jnp.array(0, dtype=jnp.bool_),
                "plate placed on counter":  jnp.array(0, dtype=jnp.bool_),
                "dish placed on counter":   jnp.array(0, dtype=jnp.bool_),
                "ingredient picked":        jnp.zeros(MAX_INGREDIENTS, dtype=jnp.bool_),
                "ingredient placed in pot": jnp.zeros(MAX_INGREDIENTS, dtype=jnp.bool_),
                "ingredient placed on counter": jnp.zeros(MAX_INGREDIENTS, dtype=jnp.bool_),
            }

            return jax.lax.cond(
                is_interact,
                _interact,
                lambda c, a: (c, (a, 0.0, zero_info)),
                carry,
                agent,
            )
        ...
        (new_grid, new_correct_delivery, reward), (new_agents, shaped_rewards, agent_infos) = (
    ...
    def process_interact(
        ...
        # Per-agent analysis info.
        # Ingredient-specific events use fixed-size arrays of shape (MAX_INGREDIENTS,)
        # so jax.lax.scan always sees the same static pytree regardless of layout.
        # Index i corresponds to ingredient i (e.g. 0=onion/yellow, 1=broccoli/green
        # for the shipped v2 layouts; further indices are zero for single-ingredient
        # layouts).
        ingredient_picked = jnp.array([
            successful_pickup * object_is_ingredient_pile
            * (interact_item == StaticObject.ingredient_pile(i))
            for i in range(MAX_INGREDIENTS)
        ], dtype=jnp.bool_)  # shape (MAX_INGREDIENTS,)

        ingredient_placed_in_pot = jnp.array([
            successful_pot_placement * (inventory == DynamicObject.ingredient(i))
            for i in range(MAX_INGREDIENTS)
        ], dtype=jnp.bool_)  # shape (MAX_INGREDIENTS,)

        ingredient_placed_on_counter = jnp.array([
            object_is_wall * object_has_no_ingredients
            * ~inventory_is_empty
            * (inventory == DynamicObject.ingredient(i))
            for i in range(MAX_INGREDIENTS)
        ], dtype=jnp.bool_)  # shape (MAX_INGREDIENTS,)

        analsis_info = {
            # scalar events
            "plate pickup":             successful_pickup * object_is_plate_pile,
            "dish picked":              successful_dish_pickup,
            "delivery":                 successful_delivery,
            "plate placed on counter":  (object_is_wall * object_has_no_ingredients
                                         * ~inventory_is_empty
                                         * (inventory == DynamicObject.PLATE)),
            "dish placed on counter":   (object_is_wall * object_has_no_ingredients
                                         * ~inventory_is_empty
                                         * inventory_is_dish),
            # per-ingredient arrays — shape (MAX_INGREDIENTS,)
            "ingredient picked":        ingredient_picked,
            "ingredient placed in pot": ingredient_placed_in_pot,
            "ingredient placed on counter": ingredient_placed_on_counter,
        }

        return new_grid, new_agent, correct_delivery, reward, shaped_reward, analsis_info
