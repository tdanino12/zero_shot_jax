...
STRIP_PAD_X      = 4     # smaller horizontal padding for the layouts strip
STRIP_PAD_TOP    = 4     # tight gap from rule to the strip (panel C only)
PANEL_PAD_TOP    = 12
PANEL_PAD_BOTTOM = 14
HEADER_TO_RULE   = 6
RULE_TO_CONTENT  = 12
...
panel_c_h = (PANEL_PAD_TOP + LABEL_FONTSIZE + HEADER_TO_RULE + 1
             + STRIP_PAD_TOP + panel_c_content_h + PANEL_PAD_BOTTOM)
panel_c_w = COL_WIDTH
...
layouts_scale = layouts_w  / layouts_w_src
...
# Panel C: layouts strip (extends to panel edges) + caption
...
new_page.merge_transformed_page(
    layouts_page,
    Transformation().scale(layouts_scale).translate(strip_x, layouts_y))
