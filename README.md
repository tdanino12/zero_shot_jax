...
In MEP all training is conducted by running a single file.

1. For symmetric layouts (e.g., cramped_room, counter_circuit, coord_ring):

```bash
python baselines/MEP_all_phases_with_checkpoints.py --seed=0 --layout=cramped_room
```

2. For non-symmetric layouts (e.g., forced_coord, asymm_advantages):
```bash
python baselines/phase_2_training_ego_opposite_with_checkpoints.py --seed=0 --layout=cramped_room
```
