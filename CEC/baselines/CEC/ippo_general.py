...
def main(config):
    ...
    if config['TRAIN_KWARGS']['finetune']:
        ...
        finetune_appendage = "_improved_fine_tune"
        if config['FCP']:
            fcp_prefix = "fcp_"
        else:
            fcp_prefix = ""
    ...
    if config['TRAIN_KWARGS']['ckpt_id'] > 103:
        ...
        if config["ENV_NAME"] == "overcooked":
            finetune_filepath += f"/{config['ENV_KWARGS']['layout']}" #f"/cramped_room_9"
        if config['FCP']:
            finetune_filepath = f"{finetune_filepath}/ikFalse/graph{config['GRAPH_NET']}"
            finetune_ckpt_num = 19 if config['ENV_NAME'] == 'ToyCoop' else 0
        else:
            #finetune_filepath = f"{finetune_filepath}/ikTrue/{config['ENV_KWARGS']['random_reset_fn']}/graph{config['GRAPH_NET']}"
            finetune_filepath = "ckpts/ippo/overcooked/counter_circuit_9/ikTrue/reset_all/graphFalse"
            finetune_ckpt_num = 0 if config['ENV_NAME'] == 'overcooked' else 0
        ...
        with open(f"{finetune_filepath}/seed{config['SEED']}_ckpt{finetune_ckpt_num}_improved.pkl", "rb") as f:  # need to resume from last checkpoint
            previous_ckpt = pickle.load(f)
            model_params = previous_ckpt['params']
            # final_update_step = previous_ckpt['final_update_step']
            final_update_step = 0
