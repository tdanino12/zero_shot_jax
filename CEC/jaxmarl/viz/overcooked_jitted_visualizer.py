...
def render_tile(obj, highlight, agent_dir_idx, agent_inv, tile_size=TILE_PIXELS):
    ...
    def render_obj(img, obj_type, color):
        ...
        def render_agent(img):
            ...
            tri_fn = rotate_fn(tri_fn, 0.5, 0.5, 0.5 * jnp.pi * agent_idx_dir_map[agent_dir_idx])
            img = fill_coords(img, tri_fn, color)

            # Render inventory item
            def render_inventory(img):
                _render_nothing = lambda x: x
                ...
                # elif inv_type == OBJECT_TO_INDEX['dish']:
                    ...
                    return img
                img = lax.cond(agent_inv == OBJECT_TO_INDEX['onion'], _render_mini_onion, _render_nothing, img)
                img = lax.cond(agent_inv == OBJECT_TO_INDEX['plate'], _render_mini_plate, _render_nothing, img)
                img = lax.cond(agent_inv == OBJECT_TO_INDEX['dish'], _render_mini_dish, _render_nothing, img)
        ...
        def render_onion_pile(img):
            fn = point_in_rect(0, 1, 0, 1)
            img = fill_coords(img, fn, COLORS["grey"])
            onion_coords = jnp.array([(0.5, 0.15), (0.3, 0.4), (0.8, 0.35), (0.4, 0.8), (0.75, 0.75)])
            ...
            carry, _ = jax.lax.scan(apply_onion, (img, onion_coords), jnp.arange(len(onion_coords)))
            img = carry[0]
            return img
        ...
        def render_plate_pile(img):
            ...
            carry, _ = jax.lax.scan(apply_plate, (img, plate_coords), jnp.arange(len(plate_coords)))
            img = carry[0]
            return img
        ...
        img = lax.cond(
            obj_type == OBJECT_TO_INDEX['goal'],
            render_goal,
            do_nothing,
            img
        )
        img = lax.cond(
            obj_type == OBJECT_TO_INDEX['agent'],
            render_agent,
            do_nothing,
            img
        )
        ...
        img = lax.cond(
            obj_type == OBJECT_TO_INDEX['onion'],
            render_onion,
            do_nothing,
            img
        )
        ...
        img = lax.cond(
            obj_type == OBJECT_TO_INDEX['plate'],
            render_plate,
            do_nothing,
            img
        )
        img = lax.cond(
            obj_type == OBJECT_TO_INDEX['dish'],
            render_dish,
            do_nothing,
            img
        )
        img = lax.cond(
            obj_type == OBJECT_TO_INDEX['pot'],
            render_pot,
            do_nothing,
            img
        )
...
def rendering_pot(obj, img):
    ...
    def render_onions(img, num_onions):
        ...
        def render_onion(carry, i):
            ...
            img = jnp.where(i < num_onions, fill_coords(img, onion_fn, COLORS["yellow"]), img)  # show progress of onions filled
            return (img, coords, num_onions), None
        carry, _ = lax.scan(render_onion, (img, onion_fns, num_onions), jnp.arange(len(onion_fns)))
        img = carry[0]
        return img

    img = lax.cond(
        (num_onions > 0) & ~is_done,
        render_onions,
        lambda x, y: x,
        img,
        num_onions
    ...
    img = lax.cond(
        is_cooking,
        lambda x: fill_coords(x, point_in_rect(0.1, 0.9-(0.9-0.1)/20*pot_status, 0.83, 0.88), COLORS["green"]),
        lambda x: x,
        img
    )
