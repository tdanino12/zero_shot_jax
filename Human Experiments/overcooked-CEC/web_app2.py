...
def initialize_user(request: Request):
    ...
    if is_mid_experiment:
        # Reconnect mid-experiment: preserve stage progress and DB rows.
        # The new task will load stage state from DB (which has correct model
        # shapes) and render into the new client's containers.
        logger.info(f"initialize_user: mid-experiment reconnect at stage {stage_idx}, keeping DB state")
        # Clear in-memory per-stage cache so nicewebrl re-loads from DB
        for stage in experiment.all_stages:
            if isinstance(getattr(stage, 'user_data', None), dict):
                stage.user_data.pop(user_seed, None)
        # Do NOT add to _fresh_start_seeds — we WANT to load from DB
    else:
