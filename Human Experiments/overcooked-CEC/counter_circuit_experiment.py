...
print(f"[counter_circuit_experiment] YAML base_config FC_DIM_SIZE={base_config.get('FC_DIM_SIZE')} GRU_HIDDEN_DIM={base_config.get('GRU_HIDDEN_DIM')}")
...
tutorial_config['ENV_KWARGS']['layout'] = 'counter_circuit_9'  # same map as experiment
...
print(f"[counter_circuit_experiment] no_graph_config FC_DIM_SIZE={no_graph_config['FC_DIM_SIZE']} GRU_HIDDEN_DIM={no_graph_config['GRU_HIDDEN_DIM']} GRAPH_NET={no_graph_config['GRAPH_NET']}")
...
print(f"[counter_circuit_experiment] ik_finetune_model FC_DIM_SIZE={ik_finetune_model.config['FC_DIM_SIZE']} id={id(ik_finetune_model.config)}")
print(f"[counter_circuit_experiment] no_graph_config id={id(no_graph_config)}")
...
MY_LAYOUT = 'counter_circuit'  # layout name used in the my/fcp checkpoint paths
...
load_path_ik = f"/home/tom.danino/crossEnvCooperation/ckpts/ippo/overcooked/counter_circuit_9/ikFalse/reset_all/graphFalse"
...
load_path_e3t = f"/home/tom.danino/crossEnvCooperation/ckpts/ippo/overcooked/counter_circuit_9/ikFalse/reset_all/graphFalse"
...
try:
    ...
    print(f"[counter_circuit_experiment] Loaded fcp checkpoint from {load_path_fcp}")
except Exception as _e:
    print(f"[counter_circuit_experiment] WARNING: Could not load fcp checkpoint: {_e}")
    param_dict["fcp"] = None
...
try:
    ...
    print(f"[counter_circuit_experiment] Loaded my checkpoint from {load_path_my}")
except Exception as _e:
    print(f"[counter_circuit_experiment] WARNING: Could not load my checkpoint: {_e}")
    param_dict["my"] = None
...
try:
    with open(f"{load_path_mep}/{SEED}/ego.pkl", "rb") as f:
        ...
        num_seed_dict["mep"] = 1
    print(f"[counter_circuit_experiment] Loaded mep checkpoint from {load_path_mep}")
except Exception as _e:
    print(f"[counter_circuit_experiment] WARNING: Could not load mep checkpoint: {_e}")
    param_dict["mep"] = None
...
try:
    ...
    print(f"[counter_circuit_experiment] Loaded hsp checkpoint from {load_path_hsp}")
except Exception as _e:
    print(f"[counter_circuit_experiment] WARNING: Could not load hsp checkpoint: {_e}")
    param_dict["hsp"] = None
...
for model_name, model in model_dict.items():
    ...
    if param_dict[model_name] is None:
        print(f"[counter_circuit_experiment] Skipping stage for '{model_name}' (no params loaded)")
        continue
