...
def make_train(config, pretrained_params):
    ...
    def train(rng):
        ...
        # agent_0: fresh random init — trained with env reward + empowerment
        train_state_0 = TrainState.create(apply_fn=network.apply, params=network.init(_rng0, init_x), tx=tx)
        # agent_1: pretrained weights — trained with env reward only
        train_state_1 = TrainState.create(apply_fn=network.apply, params=pretrained_params, tx=tx)
        '''
        ...
        tx_0 = make_tx(3e-5)        # agent_0 lr
        tx_1 = make_tx(3e-5) # agent_1 lr
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total_timesteps", type=int, default=35e6)
    ...
    parser.add_argument("--buffer_size", type=int, default=300000) #200000)
    ...
    if(args.type=="socalizer"):
        print("type is socalizer with scale:", args.emp_reward_scale, "with partner seed:", args.self_play_seed, "and training seed:",args.seed)
    #if(args.type=="achiever" and args.achiever_type == 0):
    #    print("type is achiever with mask:", args.mask, "and ent coef:", args.ent_coef)
    #if(args.type=="achiever" and args.achiever_type == 1):
    #    print("type is anti achiever with mask:", args.mask, "and ent coef:", args.ent_coef)
    #if(args.type=="myopic"):
    #    print("type is myopic with discount:", args.gamma, "and ent coef:", args.ent_coef)    
    print("#####################################")
    ...
    # Load pretrained params for agent_1
