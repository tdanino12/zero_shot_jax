...
def get_rollout(train_state_0, train_state_1, config):
    ...
    while not done:
        ...
        pi_0, _ = network.apply(train_state_0.params, obs_flat["agent_0"])
        pi_1, _ = network.apply(train_state_1.params, obs_flat["agent_1"])
...
def make_train(config):
    ...
    def train(rng):
        ...
        tx_0 = make_tx(3e-5)
        tx_1 = make_tx(config["partner_lr"])

        # Both agents initialized from scratch with independent random keys
        train_state_0 = TrainState.create(apply_fn=network.apply, params=network.init(_rng0, init_x), tx=tx_0)
        train_state_1 = TrainState.create(apply_fn=network.apply, params=network.init(_rng1, init_x), tx=tx_1)
        ...
        def _update_step(runner_state, step_idx):
            ...
            def _env_step(runner_state, unused):
                train_state_0, train_state_1, env_state, last_obs, rng = runner_state

                obs_batch = batchify(last_obs, env.agents, config["NUM_ACTORS"])
                # obs_batch[:n] = agent_0,  obs_batch[n:] = agent_1
                ...
                # Concatenate in batchify order: [agent_0..., agent_1...]
                action   = jnp.concatenate([action_0,   action_1],   axis=0)
                value    = jnp.concatenate([value_0,    value_1],    axis=0)
                log_prob = jnp.concatenate([log_prob_0, log_prob_1], axis=0)
                ...
                # agent_1: env reward + shaped reward only
                ...
                if(config["type"] == "socalizer"):
                    emp_reward = compute_empowerment_reward(
                        ...
                        obs_batch[:n],   # agent_0 current obs
                        action_0,        # agent_0 action (robot)
                        action_1,        # agent_1 action (human)
                        next_obs_batch[:n],  # agent_0 next obs
                        config["reward_type"], config["repr_dim"],
                    )
                
                    # Only apply empowerment after repr network has warmed up
                    #reward["agent_0"] =  (1-config["emp_reward_scale"])*reward["agent_0"] + config["emp_reward_scale"]*emp_reward
                    #reward["agent_0"] =  (1-config["emp_reward_scale"])*reward["agent_0"] - config["emp_reward_scale"]*emp_reward
                elif(config["type"] == "achiever"):
                    
                    penalty = jax.random.lognormal(_rng, sigma=20, shape=reward["agent_0"].shape)
                    
                    mask1 = jax.random.bernoulli(_rng, p=config["mask"], shape=reward["agent_0"].shape)
                    mask = jax.random.uniform(_rng, shape=reward["agent_0"].shape, minval=0.0, maxval=2.1)
                    
                    if(config["achiever_type"]) == 0:  # the agent is an achiever
                        reward["agent_1"] = reward["agent_1"] -mask1*reward["agent_1"]*mask
                    else:  # the agent is an anti achiever
                        reward["agent_0"] =  reward["agent_0"] -mask1*reward["agent_0"]*mask
                    ...
                    # Only apply empowerment after repr network has warmed up
                    reward["agent_0"] =  (1-config["emp_reward_scale"])*reward["agent_0"] + config["emp_reward_scale"]*emp_reward
                    reward["agent_0"] = jnp.exp(config["risk_reward_scale"]*jnp.clip(reward["agent_0"], -10, 10))
                    reward["agent_1"] += config["risk_reward_scale"]*jnp.exp(jnp.clip(reward["agent_1"], -10, 10))
                else:
                    reward["agent_0"] = reward["agent_0"]
                '''
            ...
            # Fill buffer with agent_0 trajectories from this rollout
            agent_0_obs     = traj_batch.obs[:, :n]                              # (T, n, obs_dim)
            agent_0_actions = traj_batch.action[:, :n]                           # (T, n)
            agent_1_actions = traj_batch.action[:, n:]                           # (T, n)
            ...
            last_val = jnp.concatenate([last_val_0, last_val_1], axis=0)
            ...
            def _update_epoch(update_state, unused):
                ...
                def _update_minbatch(train_states, batch_info):
                    ...
                    # first half = agent_0, second half = agent_1
                    half = traj_batch.obs.shape[0] // 2

                    def _loss_fn(params, traj_batch, gae, targets, id_ent):
                        ...
                        total_loss = loss_actor + config["vf_coef"] * value_loss - config[id_ent] * entropy
                    ...
                    _, grads_0 = grad_fn(train_state_0.params, mb_0, advantages[:half], targets[:half],"entropy_0")
                    ...
                    mb_1 = Transition(
                        ...
                        value=traj_batch.value[half:], reward=traj_batch.reward[half:],
                        log_prob=traj_batch.log_prob[half:], obs=traj_batch.obs[half:], info=None)
                    _, grads_1 = grad_fn(train_state_1.params, mb_1, advantages[half:], targets[half:],"entropy_1")
                    train_state_1 = train_state_1.apply_gradients(grads=grads_1)

                    return (train_state_0, train_state_1), None

                train_state_0, train_state_1, traj_batch, advantages, targets, rng = update_state
                rng, _rng = jax.random.split(rng)
                ...
                t0, adv0, tgt0 = make_minibatches(slice(None, n))
                t1, adv1, tgt1 = make_minibatches(slice(n, None))

                minibatches = (
                    Transition(
                        done     = jnp.concatenate([t0.done,     t1.done],     axis=1),
                        action   = jnp.concatenate([t0.action,   t1.action],   axis=1),
                        value    = jnp.concatenate([t0.value,    t1.value],    axis=1),
                        reward   = jnp.concatenate([t0.reward,   t1.reward],   axis=1),
                        log_prob = jnp.concatenate([t0.log_prob, t1.log_prob], axis=1),
                        obs      = jnp.concatenate([t0.obs,      t1.obs],      axis=1),
                        info     = None,
                    ),
                    jnp.concatenate([adv0, adv1], axis=1),
                    jnp.concatenate([tgt0, tgt1], axis=1),
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--partner_lr", type=float, default=3e-5)
    parser.add_argument("--num_envs", type=int, default=100)
    parser.add_argument("--num_steps", type=int, default=128)
    parser.add_argument("--total_timesteps", type=int, default=50e6)
    ...
    parser.add_argument("--entropy_0", type=float, default=0.1)
    parser.add_argument("--entropy_1", type=float, default=0.01)
    ...
    parser.add_argument("--type", type=str, default="achiever")
    ...
    if(args.type=="explorer"):
        print("type is explorer with entropy:", args.entropy_0)
    if(args.type=="achiever"):
        print("type is achiever with partner lr:", args.partner_lr)
    print("#####################################")
    ...
    with jax.disable_jit(False):
        # make_train no longer needs pretrained_params
    ...
    if args.save:
        ckpt = {"model": out["runner_state"][1], "config": config}
        orbax_checkpointer = orbax.checkpoint.PyTreeCheckpointer()
        save_args = orbax_utils.save_args_from_target(ckpt)
        save_path = os.path.join(os.getcwd(), "phase_1", args.layout, str(args.seed), "final", args.output)
        if not os.path.exists(save_path):
            orbax_checkpointer.save(save_path, ckpt, save_args=save_args)
            print("Saved final checkpoint.")
        else:
            print(f"Checkpoint already exists at update, resave")
            shutil.rmtree(save_path)
            orbax_checkpointer.save(save_path, ckpt, save_args=save_args)
