...
def make_phase2_train(config, env, pop_params):
    ...
    def collect_rollout(ego_state, partner_params, env_states, last_obs, last_done, hstate, rng, ego_role: int):
        """
        Collect num_steps transitions.
        ego_role=0: ego plays as agent_0, partner plays as agent_1.
        ego_role=1: ego plays as agent_1, partner plays as agent_0.
        Ego uses RNN; hstate is carried across steps and reset on episode done.
        """
        ego_key   = "agent_0" if ego_role == 0 else "agent_1"
        partner_key = "agent_1" if ego_role == 0 else "agent_0"

        def _step(carry, _):
            ego_state, env_states, last_obs, last_done, hstate, rng = carry
            rng, a_rng, s_rng = jax.random.split(rng, 3)

            obs_ego     = last_obs[ego_key]      # (num_envs, *obs_shape)
            obs_partner = last_obs[partner_key]  # (num_envs, *obs_shape)

            # Ego RNN forward pass
            obs_ego_flat    = obs_ego.reshape((num_envs, obs_dim))
            obs_ego_batched = obs_ego_flat[None]     # (1, num_envs, obs_dim)
            done_batched    = last_done[None]         # (1, num_envs)

            new_hstate, (pi_ego, val_ego) = ego_state.apply_fn(
                ego_state.params, hstate, obs_ego_batched, done_batched
            )
            ...
            pi_partner, _ = partner_network.apply(partner_params, obs_partner)
            ...
            acts = {ego_key: act_ego, partner_key: act_partner}
            s_rngs = jax.random.split(s_rng, num_envs)
            new_obs, new_env_states, rewards, dones, infos = jax.vmap(env.step)(
                s_rngs, env_states, acts
            )

            task_r   = rewards[ego_key]
            shaped_r = infos.get("shaped_reward", {}).get(ego_key, jnp.zeros(num_envs))
            ...
            transition = Transition(
                ...
                reward=total_r,
                log_prob=logp_ego,
                obs=obs_ego_flat,   # store flattened ego obs (num_envs, obs_dim)
            ...
            return carry, (transition, rewards[ego_key])
        ...
        last_obs_ego_flat = last_obs[ego_key].reshape((num_envs, obs_dim))
        _, (_, last_val) = ego_state.apply_fn(
            ego_state.params, hstate, last_obs_ego_flat[None], last_done[None]
        )
        ...
        if ep_returns is None:
            ep_returns   = (info.get(ego_key) or {}).get("returned_episode_returns", None)
            ep_done_mask = (info.get(ego_key) or {}).get("returned_episode", None)
    ...
    def train_ego(rng):
        ...
        for update_idx in range(num_updates):
            # Alternate ego role: even updates → agent_0, odd updates → agent_1
            ego_role = update_idx % 2  # 0 or 1
            ...
            (ego_state, env_states, last_obs, last_done, hstate, rng,
             traj, adv, tgt, avg_ret, ep_task_rews) = collect_rollout(
                ego_state, partner_params, env_states, last_obs, last_done, hstate, rollout_rng, ego_role
            )
            ...
            if update_idx % 50 == 0:
                print(f" Phase2 update {update_idx}/{num_updates} | "
                      f"partner={partner_idx} | ego_role=agent_{ego_role} | "
                      f"episode_return={float(avg_ret):.2f} | "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--phase1_total_timesteps", type=int, default=int(35e6))#int(35e6))
    ...
    parser.add_argument("--save_dir", type=str, default="mep_checkpoints")
    ...
    #ego_save_path = os.path.join(os.getcwd(), str(args.save_dir), "ego", str(args.seed))
