    ...
    def train(rng):
        ...
        def _update_step(runner_state, step_idx):
            ...
            def save_callback(step, params):
                step = int(step)
                ckpt = {"model": params, "config": config}
                ...
                if step == config["NUM_UPDATES"] // 3:
                    ...
                    s = "mid"
                save_path = os.path.join(os.getcwd(), "phase1_fcp", args.layout, str(args.seed), s, args.output)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total_timesteps", type=int, default=35e6)#55e6)
    ...
    if args.save:
        ...
        save_path = os.path.join(os.getcwd(), "phase1_fcp" ,args.layout, str(args.seed), f"{args.output}")
        if not os.path.exists(save_path):
