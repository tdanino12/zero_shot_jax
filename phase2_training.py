...
def make_train(config, population_params):
    ...
    def train(rng):
        ...
        partner_network_RNN = ActorCriticRNN(
            action_dim=env.action_space().n,
            hidden_size=config.get("hidden_size", 128),
            activation=config["activation"],
            obs_shape=env.observation_space().shape
        ...
        # ------------------------------------------------------------------ #
        # SHARED PPO UPDATE LOGIC
        # ------------------------------------------------------------------ #
        def _ppo_update(ego_state, traj_batch, advantages, targets, rng):
            """Shared PPO epoch/minibatch update — same for both update steps."""
        ...
        # ------------------------------------------------------------------ #
        # UPDATE STEP: RNN PARTNER (used for self-play)
        # runner_state = (ego_state, env_state, last_obs, last_done,
        #                 hstate, rng, partner_params, partner_h_state)
        # ------------------------------------------------------------------ #
        def _update_step_rnn(runner_state, unused):
            ego_state, env_state, last_obs, last_done, hstate, rng, partner_params, partner_h_state = runner_state
            ...
            def _env_step(runner_state, unused):
                ...
                # Partner forward pass — always RNN
                new_partner_hstate, (pi_partner, _) = partner_network_RNN.apply(
                    partner_params,
                    partner_h_state,
                    obs_batch[:n][None],
                    last_done[None],
                )
                pi_partner = jax.tree_util.tree_map(lambda x: x.squeeze(0), pi_partner)
                ...
                action_full = jnp.concatenate([action_partner, action_ego], axis=0)
                ...
                runner_state = (ego_state, env_state, obsv, done_ego, new_hstate, rng, partner_params, new_partner_hstate)
            ...
            _, (_, last_val) = ego_network.apply(
                ego_state.params, hstate, last_obs_batch[n:][None], last_done[None],
            )
            ...
            runner_state = (ego_state, env_state, last_obs, last_done, hstate, rng, partner_params, hstate)
        ...
        # ------------------------------------------------------------------ #
        def _update_step_mlp(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                # Partner forward pass — always MLP (stateless)
                pi_partner, _ = partner_network_mlp.apply(partner_params, obs_batch[:n])
        ...
        def _partner_phase(runner_state, partner_idx):
            ...
            inner_runner, metrics = jax.lax.scan(
                _update_step_mlp, inner_runner, None, updates_per_partner
            )
        ...
        def _population_round(runner_state, round_idx):
            ...
            # --- Self-play phase: ego vs ego (RNN partner = ego params) ---
            self_play_done   = jnp.zeros_like(last_done)
            hstate_self      = ActorCriticRNN.initialize_carry(config.get("hidden_size", 128), n)
            hstate_partner   = ActorCriticRNN.initialize_carry(config.get("hidden_size", 128), n)

            # runner for RNN update step: (..., partner_params, partner_h_state)
            self_play_runner = (ego_state, env_state, last_obs, self_play_done,
                                hstate_self, rng, ego_state.params, hstate_partner)

            self_play_runner, self_play_metrics = jax.lax.scan(
                _update_step_rnn, self_play_runner, None, updates_per_partner
            )
            ego_state, env_state, last_obs, last_done, hstate, rng, _, _ = self_play_runner
